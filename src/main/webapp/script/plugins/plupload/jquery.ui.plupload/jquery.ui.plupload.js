	...
	obj.html(
		'<div class="plupload_wrapper">' +
			'<div class="ui-widget-content plupload_container">' +
				...
				'<tr>' +
					'<td class="plupload_cell plupload_file_name">' + _('文件名称') + '</td>' +
					'<td class="plupload_cell plupload_file_status">' + _('文件状态') + '</td>' +
				...
				'<div class="plupload_content">' +
					'<div class="plupload_droptext">' + _("拖拽文件到此") + '</div>' +
				...
				'<tr>' +
					'<td class="plupload_cell plupload_file_name">' +
						'<div class="plupload_buttons"><!-- Visible -->' +
							'<a class="plupload_button plupload_add">' + _("增加文件") + '</a>&nbsp;' +
		...
		uploader.bind('Error', function(up, err) {			
			...
			switch (err.code) {
				case plupload.FILE_EXTENSION_ERROR:
					details = o.sprintf(_("文件: %s 不允许上传此类文件." ), err.file.name);
					break;
				
				case plupload.FILE_SIZE_ERROR:
					details = o.sprintf(_("文件: %s, 大小: %d, 超过允许上传文件的最大值: %d"), err.file.name,  plupload.formatSize(err.file.size), plupload.formatSize(plupload.parseSize(up.getOption('filters').max_file_size)));
					break;

				case plupload.FILE_DUPLICATE_ERROR:
					details = o.sprintf(_("%s 已经存在于上传的列表中."), err.file.name);
					break;
					
				case self.FILE_COUNT_ERROR:
					details = o.sprintf(_("最大允许上传文件数量 %d 个,多余的文件将不会被上传."), up.getOption('filters').max_file_count || 0);
					break;
				
				case plupload.IMAGE_FORMAT_ERROR :
					details = _("图片格式错误或不支持.");
					break;	
	...
	*/
	...
	_handleState: function() {
		...
		if (up.total.queued === 0) {
			$('.ui-button-text', this.browse_button).html(_('增加文件'));
		} else {
			$('.ui-button-text', this.browse_button).html(o.sprintf(_('%d 文件加入队列'), up.total.queued));
		}
